...
const ErrorResult = ({ error, ...props }: ErrorResult) => {
  ...
  return (
    <ResultBase {...props}>
      <div>
        <Span secondary>{t("errorResult.subtext")}</Span>
        <ErrorTitle>{t("errorResult.title")}</ErrorTitle>
        <Paragraph mb={2} fontWeight={"bold"}>
          {t("suggest")}
        </Paragraph>
        <Paragraph>{t("errorResult.suggest1")}</Paragraph>
        <Paragraph>{t("errorResult.suggest2")}</Paragraph>
      </div>
    </ResultBase>
  );
};
