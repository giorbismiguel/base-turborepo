...
function handlerItem(
  menu: (IMenu | IMenuItem | IMenuLeaf)[],
  t: (value: string) => string,
  hasPermission: (
    permissions: string | string[],
    atLessOne?: boolean | undefined
  ) => boolean
): (IMenu | IMenuItem | IMenuLeaf)[] {
  return menu.filter((item) => {
    const show = item?.permissions?.length
      ? hasPermission(item.permissions, item.atLessOne)
      : true;
    ...
    if ((item as IMenu).items) {
      ...
      menuItem.items = handlerItem(
        menuItem.items as IMenuItem[],
        t,
        hasPermission
      ) as IMenuItem[];
      ...
      menuItem.children = handlerItem(
        menuItem.children as IMenuLeaf[],
        t,
        hasPermission
      ) as IMenuLeaf[];
...
export const useMenu = (menu: IMenu[], translation: string = "menu") => {
  ...
  return useMemo<IMenu[]>(
    () => handlerItem(cloneDeep(menu), t, hasPermission) as IMenu[],
    [menu, t, hasPermission]
  );
};
