...
const TIMEOUT: number = Number(
  process.env.REACT_APP_TIMEOUT ||
    process.env.NEXT_PUBLIC_APP_TIMEOUT ||
    1000000
);
...
class ApiClientService {
  ...
  constructor() {
    ...
    this.REFRESH_TOKEN_KEY =
      process.env.REACT_APP_REFRESH_TOKEN || "__refresh_token__";
  ...
  setup({
    ...
    this.loginPath =
      loginPath || /(\/ms-auth\/api\/signin)|(\/api\/refresh-token)/;
  }
  ...
  handleError = async (err: DFLError) => {
    ...
    if (this.TokenService && this.getToken() && err.response) {
      const isNotLogin = this.loginPath
        ? !originalConfig.url?.match(this.loginPath)
        : false;
      if (isNotLogin) {
        debugger;
  ...
  getSpace() {
    return (
      process.env.NEXT_PUBLIC_FORCE_SPACE ||
      process.env.REACT_APP_FORCE_SPACE ||
