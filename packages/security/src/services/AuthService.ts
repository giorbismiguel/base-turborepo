...
class AuthService extends EntityApiService<any> {
  getMe = () => {
    return this.handleResponse(
      ApiClientService.get(`${API_AUTH.url}/users/me`)
    );
  };

  signIn = (params: AuthCredential): Promise<AuthResult> => {
    return this.handleResponse(
      ApiClientService.post(`${API_AUTH.url}/signin`, params)
    );
  };

  signUp = (params: any) => {
    return this.handleResponse(
      ApiClientService.post(`${API_AUTH.url}/signup`, params)
    );
  };

  resendConfirmationAccess = (params: any) => {
    return this.handleResponse(
      ApiClientService.post(`${API_AUTH.url}/confirm/resend`, params)
    );
  };

  verify = (key: string): Promise<AuthResult> => {
    return this.handleResponse(
      ApiClientService.post(`${API_AUTH.url}/confirm`, { key })
    );
  };
}
