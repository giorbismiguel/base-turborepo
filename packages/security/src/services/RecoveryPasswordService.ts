...
class RecoveryPasswordService extends EntityApiService<any> {
  ...
  init = (params: { identifier: string }) => {
    return this.handleResponse(ApiClientService.post(this.getPath(`/init`), params)).then((data) => {
  ...
  finish = (params: any) => {
    return this.handleResponse(ApiClientService.post(this.getPath(`/finish`), params));
  };

  check = (key: string, type: string = 'email') => {
    return this.handleResponse(ApiClientService.post(this.getPath(`/check`), { key, type }));
  };
