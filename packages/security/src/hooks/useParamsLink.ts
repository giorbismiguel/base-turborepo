...
export const getQueryObj = (
  ...
  return entries.reduce(
    (a: { [key: string]: string | string[] }, b: [string, string]) => {
      const key = b[0];
      const value = b[1];
      if (a[key]) {
...
export const useSearchParamsChange = (field?: string, all?: boolean) => {
  ...
  const update = useCallback(
    (params: object, removeFields?: string | string[]) => {
      const queries = getQueryObj(searchParams);
      if (removeFields)
        ...
        else delete queries[removeFields as string];
      setSearchParams({ ...queries, ...params });
    },
    [searchParams, setSearchParams]
  );
  ...
  const removeField = useCallback(
    (field: string | string[]) => {
      ...
      else delete queries[field as string];
  ...
  const fieldValue = field
    ? searchParams[all ? "getAll" : "get"](field)
    : undefined;
