...
const SelectFilter = ({id, title, options = [], onChange, value = defaultValue}: SelectFilterProps) => {
    const {t} = useTranslation('common');
    const {selected, handleSelect, handleClear} = useManySelectFilter(value, onChange);
        ...
        <FilterBase title={title} id={id} menuProps={menuProps}>
            {
                options.map(option => {
                    ...
                    const label: ReactNode = option.label || option.title || option.name || option;
                    const isSelected = selected.indexOf(itemValue) > -1;
                    const primaryLabel = option.translate ? t(label as string) : label;

                    return (<MenuItem key={itemValue} value={itemValue}
                                      selected={isSelected} onClick={() => handleSelect(itemValue)}>
