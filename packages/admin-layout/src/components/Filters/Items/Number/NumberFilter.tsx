...
import {Box, ListItemText, TextField, ToggleButton, ToggleButtonGroup} from "@mui/material";
...
const NumberFilter = ({id, title, onChange, value = defaultValue}: NumberFilterProps) => {
    const {t} = useTranslation('common');
    const {rangeValue, handleChange, handleClear, handleChangeType} = useRangeFilter(value, onChange);
            ...
            <Box px={2} py={1}>
                <FlexBox justifyContent={"center"} mb={2}>
                    <ToggleButtonGroup
                        ...
                        aria-label="Platform"
                    >
                        <ToggleButton value={false} sx={{minWidth: 120}}>{t('filtersCom.exact')}</ToggleButton>
                        <ToggleButton value={true} sx={{minWidth: 120}}>{t('filtersCom.range')}</ToggleButton>
                    </ToggleButtonGroup>
                </FlexBox>
                <ConditionContainer active={rangeValue.isRange} alternative={<TextField size={'small'}
                                                                                        name={'equal'}
