...
import {defaultValue, isRangeEqual, rangeToString, stringToRange} from "./parser";


export const useRangeFilter = (value: string, onChange: (value: string) => void) => {
    ...
    const handleChange = useCallback(({target: {value, name}}: any) => {
        setRange(prevState => {
    ...
    const handleChangeType = useCallback((
        ...
        setRange(prevState => {
            ...
            if (!isRangeEqual(newValue, prevState))
                update(rangeToString(newValue))
            return newValue;
        });
    }, [update]);
