...
export const useDateRangeFilter = (
  ...
  const [rangeValue, setRange] = useState<Date[] | string | undefined>(
    defaultValue
  );
  const [customRangeValue, setCustomRangeValue] =
    useState<IDateSelection>(selectionRange);
  ...
  const handleSelectedData = useCallback(
    (item: Date[] | string) => {
      ...
      if (typeof item === "string") {
        update(item);
      } else update(rangeToString(item));
    },
    [update]
  );
  ...
  const mOptions = useMemo(
    () =>
      options?.map((option: IDateRangeOption | string) => {
        if (typeof option === "string") {
          return { _id: option, name: t(option) };
        }
        return {
          ...option,
          name: option.label || option.name || option._id || "",
        };
      }),
    [options, t]
  );
