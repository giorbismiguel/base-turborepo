...
export default function Table({
  ...
  const { selected, handleSelectAll, handleSelectionClick, isSelected } =
    useTableSelection(data);
  const { page, rowsPerPage, onRowsPerPageChange, onPageChange } =
    useTablePagination();
    ...
    <Box sx={{ width: "100%" }}>
      <TableContainer>
        ...
        >
          ...
          <TableBody>
            {isLoading && (
              <LoadingRows headCellsSize={columns.length} select={select} />
            )}
            {!isLoading &&
                    ...
                    {select && (
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          onChange={(event) =>
                            select && handleSelectionClick(event, id)
                          }
                          inputProps={{
