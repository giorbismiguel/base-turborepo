...
const TabsFilter = ({
  ...
  const tabs = useMemo(() => {
    ...
    for (let view in views) {
      ...
      const show = viewValue?.permissions?.length
        ? hasPermission(viewValue.permissions, viewValue.atLessOne)
        : true;
      if (!show) return;

      tabs.push(<Tab label={t(viewValue.title)} value={view} key={view} />);
    }
    return tabs;
